                ...
                for step, (inputs, targets, _) in enumerate(train_dataloader):
                    ...
                    with torch.autocast(
                        ...
                        if torch.isnan(inputs).any():
                            logger.warning(f"NaN in inputs at epoch {epoch}, step {step}")
                        if torch.isnan(targets).any():
                            logger.warning(f"NaN in targets at epoch {epoch}, step {step}")
                        if torch.isnan(outputs[0]).any():
                            logger.warning(f"NaN in model output at epoch {epoch}, step {step}")
                        if torch.isinf(outputs[0]).any():
                            logger.warning(f"Inf in model output at epoch {epoch}, step {step}")
