...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels, dilations=(1, 6, 12, 18)):
        ...
        for dilation in dilations:
            self.blocks.append(
                nn.Conv2d(in_channels, out_channels, kernel_size=3 if dilation > 1 else 1,
                          stride=1, padding=dilation if dilation > 1 else 0,
    ...
    def forward(self, x):
        ...
        for conv in self.blocks:
            out.append(conv(x))
        out = torch.cat(out, dim=1)  # concat along channel
        out = self.bn(out)
        out = self.act(out)
