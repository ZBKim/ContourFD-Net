...
nan_log_path = "/raid/jzb/ContourFD-Net/ContourFD-Net/src/nan_files.txt"
...
class DSB2018Dataset(BaseDataset):
    def __getitem__(
        ...
        if np.isnan(x).any() or np.isnan(y).any():
            print(f"[NaN Warning] NaN detected at index {index}, skipping: {self.X[index]}")
            log_nan(self.X[index])

            img = load_img(self.X[index])
            print("Nan count:", np.isnan(img).sum())
            print("Inf count:", np.isinf(img).sum())
